...
// Referência ao Socket.IO (injetada após setupSocketIO)
let _io = null;

// Mapa para associar o Nome da Conta (usado como sessionId pelos módulos antigos) ao ID da Conta
const _accountIdMap = new Map();

/**
 * Injeta instância do Socket.IO para log em tempo real
 */
function setIO(io) {
    _io = io;
}
...
/**
 * Registra o ID de uma conta para associar com seu nome nos logs.
 */
function setAccountId(accountName, accountId) {
    if (accountName && accountId) {
        _accountIdMap.set(accountName, accountId);
    }
}
...
function log(level, sessionId, message, color = colors.reset) {
    ...
    // Emite log em tempo real via Socket.IO (apenas logs de contas específicas)
    if (_io && sessionId) {
        const accountId = _accountIdMap.get(sessionId); // Tenta pegar o ID mapeado

        _io.emit('account:log', {
            accountName: sessionId,
            accountId: accountId || sessionId, // Manda o ID se existir, ou o próprio nome como fallback
            level,
            message,
            timestamp
        });
